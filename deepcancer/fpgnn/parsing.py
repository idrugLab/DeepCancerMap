...
from deepcancer.fpgnn.features import get_available_features_generators
from deepcancer.fpgnn.utils import makedirs

"""
argparse三步：
创建ArgumentParser对象
add_argument()给对象添加argument解析规则
调用parse_args()解析传入的内容，生成Namespace对象
"""
...
def add_predict_args(parser: ArgumentParser):
    ...
    parser.add_argument('--gpu', type=int,
                        choices=list(range(torch.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")).device_count())),
...
def add_train_args(parser: ArgumentParser):
    ...
    # Model arguments
    parser.add_argument('--fp_2_dim', type=int, default=512,
                        help='dim of the second fpn dim')
    parser.add_argument('--nhid', type=int, default=60,
                        help='dim of gat atom hidden layer')
    parser.add_argument('--nheads', type=int, default=8,
                        help='number of gat attentions')
    parser.add_argument('--gat_scale', type=float, default=0.5,
                        help='scale of gat in fp-gnn')
    parser.add_argument('--dropout_gat', type=float, default=0.0,
                        help='Dropout probability of GAT')
    ...
    parser.add_argument('--dropout', type=float, default=0.0,
                        help='Dropout probability of FPN and FFN')
    ...
    parser.add_argument('--atom_messages', action='store_true', default=True,
                        help='Use messages on atoms instead of messages on bonds =True用点信息  =False用边信息 初始为default=False 此处改为True')


def update_checkpoint_args(args: Namespace):
    """
    Walks the checkpoint directory to find all checkpoints, updating args.checkpoint_paths and args.ensemble_size.
    遍历检查点目录以找到所有检查点，更新args.checkpoint_paths和args.ensemble_size
...
def modify_predict_args(args: Namespace):
    ...
    #assert aaa 断言 debug工具 检查先验条件使用断言，检查后验条件使用异常  比如此处，断言用来在aaa的=0时抛出异常
...
def modify_train_args(args: Namespace):
    ...
    :param args: Arguments.
    """
    global temp_dir  # Prevents the temporary directory from being deleted upon function return 防止在函数返回时删除临时目录
...
def parse_train_args() -> Namespace:
    ...
    add_train_args(parser)#添加解析规则，在此分析传入的各个命令行输入，和模型超参数
    args = parser.parse_args()#调用解析规则，返回填充好后的对象
