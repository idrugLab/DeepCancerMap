...
from deepcancer.fpgnn.features import load_features
...
def get_task_names(path: str, use_compound_names: bool = False) -> List[str]:
    ...
    #index缺省为1.如果第一列不是smile，而是多了一个分子名称列，那么index为2.总之指向smile列
    task_names = get_header(path)[index:]
    #task_names=除smile列之后所有列，即靶标名称
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    for datapoint in data:
        ...
        if datapoint.smiles != '' and datapoint.mol is not None and datapoint.mol.GetNumHeavyAtoms() > 0:
            ...
            print('!!!skip_smiles:',skipnum)#输出无效原子编号，序号从1开始
    #smiles非空且存在 datapoint.mol且datapoint.mol.GetNumHeavyAtoms>0重原子数大于0
    """
...
def get_data(path: str,
    ...
    :param path: Path to a CSV file.
    :param skip_invalid_smiles: Whether to skip and filter out invalid smiles.跳过无效smiles
    ...
    if args is not None:
        # Prefer explicit function arguments but default to args if not provided 首选显式函数参数，但如果未提供，则默认为args
    ...
    skip_smiles = set()#set() 函数创建一个无序不重复元素集
    ...
    with open(path) as f:
        ...
        data = MoleculeDataset([
            ...
            ) for i, line in enumerate(lines)#for i, line in tqdm(enumerate(lines), total=len(lines))
            # tqdm(enumerate(lines), total=len(lines))其实等于 tqdm(enumerate(lines)) total=是额外修饰tqdm的
        ])
    ...
    if skip_invalid_smiles:
        ...
        if len(data) < original_data_len:
            ...
            info(f'Warning: {original_data_len - len(data)} SMILES are invalid.')
