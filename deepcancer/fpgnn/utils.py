...
from deepcancer.fpgnn.models import build_model, MoleculeModel
from deepcancer.fpgnn.nn_utils import NoamLR
...
def makedirs(path: str, isfile: bool = False):
    ...
    """
    """
    创建一个目录，该目录具有目录或文件的路径。

     如果提供了目录，请创建该目录。 如果提供了文件（即isfiled == True），
     为该文件创建父目录。

     ：param path：目录或文件的路径。
     ：param isfile：提供的路径是目录还是文件。
    """
    if isfile:
...
     ：param quiet：流处理程序是否应该保持安静（即仅打印重要信息）。
     ：return：记录器。
    """
    
    logger = logging.getLogger(name)
    #设定logger日志信息打印在控制台的程度。debug<info<warning<error<CRITICAL
    #原句logger.setLevel(logging.DEBUG)
    logger.setLevel(logging.DEBUG)
    logger.propagate = False

    # Set logger depending on desired verbosity 流输出，即控制台
    ch = logging.StreamHandler()
    if quiet:
        ch.setLevel(logging.INFO)
    else:
        ch.setLevel(logging.DEBUG)#一大段是info，进程百分比不是
    logger.addHandler(ch)

    if save_dir is not None:#文件输出
        makedirs(save_dir)
