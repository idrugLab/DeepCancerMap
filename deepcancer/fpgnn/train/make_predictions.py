...
from deepcancer.fpgnn.data.utils import get_data, get_data_from_smiles
from deepcancer.fpgnn.utils import load_args, load_checkpoint, load_scalers
...
def make_predictions(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    ...
    if args.gpu is not None:
        torch.to(torch.device("cuda" if torch.cuda.is_available() else "cpu")).set_device(args.gpu)
