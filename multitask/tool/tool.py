...
from multitask.data import MoleDataSet, MoleData, scaffold_split
from multitask.model import FPGNN
...
def load_data(path, args):
    with open(path) as file:
        ...
        if fir_data_len - now_data_len > 0:
            print('There are ', fir_data_len, ' smiles first, but ',
                  fir_data_len - now_data_len, ' smiles is invalid.  ')

    return data_val


def load_data_from_smiles(smiles, log=None, args=None):
    if log is not None:
        debug = log.debug
    else:
        debug = print

    data = MoleDataSet([MoleData(line=[smile], args=args) for smile in smiles])
...
def split_data(data, type, size, seed, log):
    ...
    if type == 'random':
        ...
        return MoleDataSet(train_data), MoleDataSet(val_data), MoleDataSet(
            test_data)
    elif type == 'scaffold':
...
class NoamLR(_LRScheduler):
    ...
    def __init__(self, optimizer, warmup_epochs, total_epochs, steps_per_epoch,
        ...
        self.linear_increment = (self.max_lr -
                                 self.init_lr) / self.warmup_steps
